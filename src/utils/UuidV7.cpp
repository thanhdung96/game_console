...
std::string UuidV7Generator::generate() {
    ...
    uint64_t unix_ts_ms = std::chrono::duration_cast<std::chrono::milliseconds>(duration).count();
    ...
    uuid_bytes[6] = ((0x7 << 4) | ((rand_a >> 8) & 0x0F)); // Version 7 (0111) and high 4 bits of rand_a
    // Byte 7: rand_a_low_8_bits
    uuid_bytes[7] = rand_a & 0xFF; // Low 8 bits of rand_a

    // Generate 62 bits for rand_b
    uint64_t rand_b = random_part_a_b & 0x3FFFFFFFFFFFFFFFULL; // Low 62 bits for rand_b

    // Byte 8: (variant << 6) | (rand_b_high_6_bits)
    uuid_bytes[8] = ((0x2 << 6) | ((rand_b >> 56) & 0x3F)); // Variant (10) and high 6 bits of rand_b
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        if (i == 3 || i == 5 || i == 7 || i == 9) { // Standard UUID hyphen positions
            ss << "-";
        }
    }
